Describe 'Write-Status' {
    ...
    Context 'Functionality tests' {
        ...
        It 'ignores pipeline input when Message parameter is set' {
            'ignored' | Write-Status -Level INFO -Message 'explicit'
            (Get-Content $script:StatusLogFile | Select-Object -Last 1) | Should -Match 'explicit'
        }
    ...
    Context 'Edge case tests' {
        ...
        It 'logs multiple messages individually' {
            Write-Status -Level INFO -Message 'one'
            Write-Status -Level INFO -Message 'two'
